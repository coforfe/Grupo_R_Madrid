...
gray_scott_run_steps_r <- function(u, v, Du, Dv, f, k, dt = 1.0, steps = 1L) {
  result <- list(u = u, v = v)
  for (i in seq_len(steps)) {
    result <- gray_scott_step_r(result$u, result$v, Du, Dv, f, k, dt)
  }
  result
}
...
compile_gray_scott_cpp <- function() {
  ...
  cppFunction('
    Rcpp::List gray_scott_run_steps_cpp(const Rcpp::NumericMatrix& u,
      ...
      const int n = u.nrow();
      const int m = u.ncol();
      std::vector<double> u_current(u.begin(), u.end());
      std::vector<double> v_current(v.begin(), v.end());
      std::vector<double> u_next(n * m);
      std::vector<double> v_next(n * m);
      ...
      for (int step = 0; step < steps; ++step) {
        for (int i = 0; i < n; ++i) {
          ...
          for (int j = 0; j < m; ++j) {
            ...
            const int idx = i + n * j;
            const int idx_up = up + n * j;
            const int idx_down = down + n * j;
            const int idx_left = i + n * left;
            const int idx_right = i + n * right;

            const double u_center = u_current[idx];
            const double v_center = v_current[idx];

            const double Lu = u_current[idx_up] + u_current[idx_down] + u_current[idx_left] + u_current[idx_right] - 4.0 * u_center;
            const double Lv = v_current[idx_up] + v_current[idx_down] + v_current[idx_left] + v_current[idx_right] - 4.0 * v_center;
      ...
      Rcpp::NumericMatrix u_result(n, m);
      Rcpp::NumericMatrix v_result(n, m);
      std::copy(u_current.begin(), u_current.end(), u_result.begin());
      std::copy(v_current.begin(), v_current.end(), v_result.begin());
...
gray_scott_run_steps <- if (exists("gray_scott_run_steps_cpp", mode = "function")) {
  function(u, v, Du, Dv, f, k, dt = 1.0, steps = 1L) {
    gray_scott_run_steps_cpp(u, v, Du, Dv, f, k, dt, as.integer(steps))
  }
} else {
  gray_scott_run_steps_r
}
...
translations <- list(
  es = list(
    ...
    explanation = list(
      ...
      p_4 = "Esta version usa un nucleo numerico compilado con Rcpp/C++ para acelerar el paso Gray-Scott, mientras mantiene la misma estructura reactiva de Shiny.",
      p_5 = "Si aun asi en tu equipo se quedara corta, el siguiente paso natural seria reducir grid y pasos por tick, o llevar mas partes del flujo al lado compilado."
    ),
    controls = list(
  ...
  en = list(
    ...
    explanation = list(
      ...
      p_4 = "This version uses an Rcpp/C++ numerical core to speed up the Gray-Scott step while keeping the same Shiny reactive structure.",
      p_5 = "If performance is still limiting on your machine, the next natural step would be to reduce grid size and steps per tick, or move more of the workflow into compiled code."
    ),
    controls = list(
      grid_size = "Grid size",
