# =============================================================================
# Gray-Scott Reaction-Diffusion System - Bilingual Fast-Render Turbo Gallery Shiny App
# =============================================================================
...
translations <- list(
  es = list(
    ...
    controls = list(
      ...
      refresh_ms = "Milisegundos por refresco",
      palette = "Paleta",
  ...
  en = list(
    ...
    controls = list(
      ...
      refresh_ms = "Milliseconds per refresh",
      palette = "Palette",
      reset = "Reset gallery",
      run = "▶ Run all",
      pause = "⏸ Pause all",
...
server <- function(input, output, session) {
        ...
        div(
          ...
          fluidRow(
            ...
            column(
              ...
              sliderInput(
                "refresh_ms",
                txt$controls$refresh_ms,
                min = 30,
                max = 200,
                value = current_or(isolate(input$refresh_ms), 60),
                step = 10
              )
            ),
            column(
  ...
  output$gallery_status <- renderText({
    ...
    paste0(
      status,
      " | ",
      txt$controls$common_iteration,
      ": ",
      first_iteration,
      " | ",
      txt$controls$refresh_value,
      ": ",
      input$refresh_ms,
      " ms"
    )
  ...
  observe({
    ...
    invalidateLater(current_or(input$refresh_ms, 60), session)
