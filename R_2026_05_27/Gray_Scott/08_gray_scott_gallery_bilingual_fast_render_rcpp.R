# =============================================================================
# Gray-Scott Reaction-Diffusion System - Bilingual Fast-Render Rcpp Gallery Shiny App
# =============================================================================
...
gray_scott_step_r <- function(u, v, Du, Dv, f, k, dt = 1.0) {
  n <- nrow(u)
  m <- ncol(u)
...
gray_scott_backend <- "R"

compile_gray_scott_cpp <- function() {
  old_path <- Sys.getenv("PATH", unset = "")
  old_makevars <- Sys.getenv("R_MAKEVARS_USER", unset = "")
  temp_makevars <- tempfile("makevars_")
  file.create(temp_makevars)

  Sys.setenv(
    PATH = "/usr/bin:/bin:/usr/sbin:/sbin:/Library/Frameworks/R.framework/Resources/bin",
    R_MAKEVARS_USER = temp_makevars
  )

  on.exit({
    Sys.setenv(PATH = old_path)
    if (nzchar(old_makevars)) {
  ...
  cppFunction('
    Rcpp::List gray_scott_step_cpp(const Rcpp::NumericMatrix& u,
      ...
      Rcpp::NumericMatrix u_new(n, m);
      Rcpp::NumericMatrix v_new(n, m);

      for (int i = 0; i < n; ++i) {
        const int up = (i == 0) ? (n - 1) : (i - 1);
        const int down = (i == n - 1) ? 0 : (i + 1);

        for (int j = 0; j < m; ++j) {
          const int left = (j == 0) ? (m - 1) : (j - 1);
          const int right = (j == m - 1) ? 0 : (j + 1);

          const double u_center = u(i, j);
          const double v_center = v(i, j);

          const double Lu = u(up, j) + u(down, j) + u(i, left) + u(i, right) - 4.0 * u_center;
          const double Lv = v(up, j) + v(down, j) + v(i, left) + v(i, right) - 4.0 * v_center;
          const double uvv = u_center * v_center * v_center;

          double next_u = u_center + dt * (Du * Lu - uvv + f * (1.0 - u_center));
          double next_v = v_center + dt * (Dv * Lv + uvv - (f + k) * v_center);

          if (next_u < 0.0) next_u = 0.0;
          if (next_u > 1.0) next_u = 1.0;
          if (next_v < 0.0) next_v = 0.0;
          if (next_v > 1.0) next_v = 1.0;
...
try({
  compile_gray_scott_cpp()
  gray_scott_backend <- "Rcpp/C++"
}, silent = TRUE)

gray_scott_step <- if (exists("gray_scott_step_cpp", mode = "function")) {
  function(u, v, Du, Dv, f, k, dt = 1.0) {
    gray_scott_step_cpp(u, v, Du, Dv, f, k, dt)
  }
} else {
  gray_scott_step_r
}
...
translations <- list(
  es = list(
    ...
    explanation = list(
      ...
      p_4 = "Esta version combina render rapido con un nucleo numerico compilado en Rcpp/C++, manteniendo la misma estructura reactiva de Shiny.",
      p_5 = "Si aun asi en tu equipo se quedara corta, el siguiente paso natural seria reducir grid y pasos por tick, o mover mas partes del flujo al lado compilado."
    ),
    controls = list(
      ...
      backend = "Backend",
      backend_cpp = "Rcpp/C++ activo",
      backend_r = "R base (fallback; C++ no disponible)"
    ),
    preset_titles = list(
  ...
  en = list(
    ...
    explanation = list(
      ...
      p_4 = "This version combines fast rendering with an Rcpp/C++ numerical core while keeping the same Shiny reactive structure.",
      p_5 = "If performance still becomes limiting on your machine, the next natural step would be to reduce grid size and steps per tick, or move more of the workflow to compiled code."
    ),
    controls = list(
      grid_size = "Grid size",
      ...
      preset_original = "Gray-Scott Original",
      backend = "Backend",
      backend_cpp = "Rcpp/C++ active",
      backend_r = "Base R (fallback; C++ unavailable)"
    ),
    preset_titles = list(
...
get_palette_values <- function(palette_name, n = 256) {
  cache_key <- paste0(palette_name, "_", n)

  if (!exists(cache_key, envir = palette_cache, inherits = FALSE)) {
    palette_values <- if (palette_name == "grayscott") {
      grayscott_palette(n)
    } else {
      viridis(n, option = palette_name)
    }

    assign(cache_key, palette_values, envir = palette_cache)
  }

  get(cache_key, envir = palette_cache, inherits = FALSE)
}

build_raster_image <- function(v_matrix, palette_name) {
  values <- pmin(1, pmax(0, v_matrix))
  palette_values <- get_palette_values(palette_name)
  color_index <- floor(values * (length(palette_values) - 1)) + 1L
  color_matrix <- matrix(
  ...
  as.raster(color_matrix[nrow(color_matrix):1, , drop = FALSE])
}

build_plot <- function(v_matrix, palette_name) {
  old_par <- par(no.readonly = TRUE)
  on.exit(par(old_par), add = TRUE)

  raster_image <- build_raster_image(v_matrix, palette_name)

  par(
    mar = c(1.2, 1.2, 2.8, 1.2),
    xaxs = "i",
    yaxs = "i",
    bg = "#fffdf9"
  )
  ...
  rasterImage(raster_image, 0, 0, 1, 1, interpolate = FALSE)
  box(col = "#d9c7b0", lwd = 0.8)
}
...
preset_card <- function(id, title, params_text) {
  div(
    ...
    div(class = "preset-iteration", textOutput(outputId = paste0("iter_", id), inline = TRUE)),
...
ui <- fluidPage(
  ...
  div(
    ...
    div(
      ...
      div(class = "backend-note", textOutput("backend_status", inline = TRUE))
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$backend_status <- renderText({
    txt <- lang_text()
    backend_label <- if (identical(gray_scott_backend, "Rcpp/C++")) {
      txt$controls$backend_cpp
    } else {
      txt$controls$backend_r
    }
    paste0(txt$controls$backend, ": ", backend_label)
  })
  ...
  for (id in preset_ids) {
    local({
      ...
      output[[paste0("iter_", local_id)]] <- renderText({
        req(state$simulations)
        txt <- lang_text()
        sim <- state$simulations[[local_id]]
        paste0(txt$controls$plot_iteration, ": ", sim$iteration)
      })
